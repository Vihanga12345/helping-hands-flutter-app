...
BEGIN
    ...
    LOOP
        BEGIN
            ...
            RAISE NOTICE '🗑️ Dropped function get_helper_statistics(%s)', func_record.args;
        EXCEPTION WHEN OTHERS THEN
            RAISE NOTICE '⚠️ Could not drop function get_helper_statistics(%s): %s', func_record.args, SQLERRM;
        END;
    END LOOP;
END $$;
...
BEGIN
    ...
    LOOP
        BEGIN
            ...
            RAISE NOTICE '🗑️ Dropped function get_helpee_statistics(%s)', func_record.args;
        EXCEPTION WHEN OTHERS THEN
            RAISE NOTICE '⚠️ Could not drop function get_helpee_statistics(%s): %s', func_record.args, SQLERRM;
        END;
    END LOOP;
END $$;
...
-- Success message
SELECT '🚨 FIXED URGENT ISSUE COMPLETE! 🚨' as status,
