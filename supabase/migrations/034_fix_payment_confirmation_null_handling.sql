...
BEGIN
  ...
  IF job_record.total_time_seconds IS NULL OR job_record.hourly_rate IS NULL THEN
    RETURN QUERY SELECT 0, 'Invalid job data: missing time or rate', 0::numeric, NULL::uuid, NULL::text, NULL::text, NULL::uuid, NULL::text, NULL::text;
    RETURN;
  END IF;
