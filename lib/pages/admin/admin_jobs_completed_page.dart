...
class AdminJobsCompletedPage extends StatefulWidget {
  const AdminJobsCompletedPage({super.key});

  @override
  State<AdminJobsCompletedPage> createState() => _AdminJobsCompletedPageState();
}

class _AdminJobsCompletedPageState extends State<AdminJobsCompletedPage> {
  ...
  Future<void> _loadCompletedJobs() async {
    try {
      ...
      final jobs = await _adminDataService.getCompletedJobs();
  ...
  List<Map<String, dynamic>> get _filteredJobs {
    if (_searchQuery.isEmpty) return _completedJobs;
        ...
        child: Column(
          ...
          children: [
            ...
            Row(
              children: [
                Text(
                  'Found ${filteredJobs.length} completed jobs',
                  style: const TextStyle(
                    fontSize: 18,
                    fontWeight: FontWeight.bold,
  ...
  Widget _buildJobCard(Map<String, dynamic> job, {required bool isGrid}) {
    ...
    final String helperName = job['helper_name'] ?? 'Unknown Helper';
    ...
    final double? rating = job['helpee_rating']?.toDouble();
    final DateTime? completedAt = job['completed_at'] != null
        ? DateTime.tryParse(job['completed_at'].toString())
        : null;
    final int? durationMinutes = job['duration_minutes']?.toInt();
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            // Header with status, privacy badge, and rating
            Row(
              children: [
                Container(
                  padding:
            ...
            Row(
              children: [
                ...
                if (durationMinutes != null && fee != null) ...[
            ...
            if (completedAt != null) ...[
              ...
              Row(
                children: [
                  ...
                  const SizedBox(width: 8),
                  Text(
                    'Completed ${DateTimeHelpers.getTimeAgo(completedAt)}',
                    style: TextStyle(
                      fontSize: isGrid ? 10 : 12,
                      color: AppColors.textSecondary,
                    ),
