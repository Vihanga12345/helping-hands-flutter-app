...
class HelpeePaymentConfirmationPage extends StatefulWidget {
  final String jobId;

  const HelpeePaymentConfirmationPage({
    super.key,
    required this.jobId,
  });

  @override
  State<HelpeePaymentConfirmationPage> createState() =>
      _HelpeePaymentConfirmationPageState();
}

class _HelpeePaymentConfirmationPageState
    extends State<HelpeePaymentConfirmationPage> {
  ...
  Future<void> _handlePaymentConfirmation() async {
    ...
    try {
      ...
      final success = await CashPaymentService.confirmHelpeePayment(
        widget.jobId,
        currentUser['user_id'],
      );
      ...
      if (success) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text(
                'Payment confirmation sent! Waiting for helper confirmation...'),
            backgroundColor: AppColors.success,
          ),
        );

        // Navigate to rating page
        context.go('/helpee/rating/${widget.jobId}');
      } else {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Failed to confirm payment. Please try again.'),
  ...
  Widget _buildPaymentConfirmationContent() {
    final helperName = _jobDetails?['helper'] != null
        ? '${_jobDetails!['helper']['first_name']} ${_jobDetails!['helper']['last_name']}'
        : 'the helper';
      ...
      child: Column(
        ...
        children: [
          ...
          Container(
            ...
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 8),
                Text(
                  'Your job has been completed by $helperName',
              ...
              children: [
                ...
                Row(
                  children: [
                    ...
                    Text(
                      'LKR ${_jobDetails!['hourly_rate'] ?? 0.0}/Hr',
                      style: AppTextStyles.bodyMedium.copyWith(
                        color: AppColors.textPrimary,
                        fontWeight: FontWeight.w600,
                      ),
          ...
          Container(
            ...
            child: Text(
              'Please pay the amount to the helper',
              style: AppTextStyles.bodyLarge.copyWith(
