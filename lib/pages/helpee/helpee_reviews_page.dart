...
class HelpeeReviewsPage extends StatefulWidget {
  const HelpeeReviewsPage({super.key});

  @override
  State<HelpeeReviewsPage> createState() => _HelpeeReviewsPageState();
}

class _HelpeeReviewsPageState extends State<HelpeeReviewsPage> {
  ...
  Future<void> _loadReviews() async {
    try {
      ...
      print('🔍 Loading reviews for helpee: $helpeeId');

      // Get reviews and statistics in parallel
      final results = await Future.wait([
        _helperDataService.getHelpeeAllReviews(helpeeId),
  ...
  Widget _buildStatisticsSummary() {
    ...
    final totalJobs = _statistics?['total_jobs'] ?? 0;
      ...
      return Container(
        ...
        child: Column(
          children: [
            ...
            const SizedBox(height: 8),
            Text(
              'Complete more jobs to receive reviews from helpers'.tr(),
      ...
      child: Column(
        ...
        children: [
          Padding(
            padding: const EdgeInsets.all(20),
            child: Text(
              'All Reviews (${_reviews!.length})'.tr(),
              style: AppTextStyles.heading3.copyWith(
                fontWeight: FontWeight.w700,
                color: AppColors.textPrimary,
              ),
