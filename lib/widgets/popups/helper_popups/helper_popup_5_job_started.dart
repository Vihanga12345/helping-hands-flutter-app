...
class HelperPopup5JobStarted extends StatefulWidget {
  final VoidCallback? onClose;
  const HelperPopup5JobStarted({Key? key, this.onClose}) : super(key: key);
  @override
  State<HelperPopup5JobStarted> createState() => _HelperPopup5JobStartedState();
}

class _HelperPopup5JobStartedState extends State<HelperPopup5JobStarted> with TickerProviderStateMixin {
      ...
      return Opacity(opacity: _opacity.value, child: Container(width: double.infinity, height: double.infinity, color: Colors.black.withOpacity(0.3),
        child: Center(child: Transform.scale(scale: _scale.value, child: Container(width: 340, height: 295,
          ...
          child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
            ...
            Padding(padding: const EdgeInsets.symmetric(horizontal: 40), child: Text('Job has been Started', textAlign: TextAlign.center, style: TextStyle().copyWith(fontSize: 24, color: Colors.black, height: 1.3))),
  ...
  static void show(BuildContext context) => showDialog(context: context, barrierDismissible: false, barrierColor: Colors.transparent, builder: (context) => HelperPopup5JobStarted(onClose: () => Navigator.of(context).pop()));
} 
