...
class HelperPopup9JobPayment extends StatefulWidget {
  final VoidCallback? onClose;
  const HelperPopup9JobPayment({Key? key, this.onClose}) : super(key: key);
  @override
  State<HelperPopup9JobPayment> createState() => _HelperPopup9JobPaymentState();
}

class _HelperPopup9JobPaymentState extends State<HelperPopup9JobPayment> with TickerProviderStateMixin {
      ...
      return Opacity(opacity: _opacity.value, child: Container(width: double.infinity, height: double.infinity, color: Colors.black.withOpacity(0.3),
        child: Center(child: Transform.scale(scale: _scale.value, child: Container(width: 340, height: 295,
          ...
          child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
            Container(width: 80, height: 80, decoration: BoxDecoration(color: AppColors.primaryGreen, shape: BoxShape.circle, boxShadow: [BoxShadow(color: AppColors.primaryGreen.withOpacity(0.3), blurRadius: 8, offset: const Offset(0, 3), spreadRadius: 2)]),
              child: const Icon(Icons.payment_rounded, color: Colors.white, size: 50)),
            const SizedBox(height: 30),
            Padding(padding: const EdgeInsets.symmetric(horizontal: 40), child: Text('Payment Received Successfully', textAlign: TextAlign.center, style: TextStyle().copyWith(fontSize: 24, color: Colors.black, height: 1.3))),
  ...
  static void show(BuildContext context) => showDialog(context: context, barrierDismissible: false, barrierColor: Colors.transparent, builder: (context) => HelperPopup9JobPayment(onClose: () => Navigator.of(context).pop()));
} 
