...
class HelpeePopup6JobPaused extends StatefulWidget {
  final VoidCallback? onClose;
  const HelpeePopup6JobPaused({Key? key, this.onClose}) : super(key: key);
  @override
  State<HelpeePopup6JobPaused> createState() => _HelpeePopup6JobPausedState();
}

class _HelpeePopup6JobPausedState extends State<HelpeePopup6JobPaused> with SingleTickerProviderStateMixin {
                ...
                child: Container(
                  ...
                  child: Column(
                    ...
                    children: [
                      Container(
                        width: 80, height: 80,
                        decoration: BoxDecoration(color: Colors.orange, shape: BoxShape.circle, boxShadow: [BoxShadow(color: Colors.orange.withOpacity(0.3), blurRadius: 8, offset: const Offset(0, 3), spreadRadius: 2)]),
                        child: const Icon(Icons.pause_rounded, color: Colors.white, size: 50),
                      ),
                      const SizedBox(height: 30),
                      Padding(
                        padding: const EdgeInsets.symmetric(horizontal: 40),
                        child: Text('Job has been Paused', textAlign: TextAlign.center, style: TextStyle().copyWith(fontSize: 24, color: Colors.black, height: 1.3)),
                      ),
  ...
  static void show(BuildContext context) {
    showDialog(context: context, barrierDismissible: false, barrierColor: Colors.transparent, builder: (BuildContext context) => HelpeePopup6JobPaused(onClose: () => Navigator.of(context).pop()));
  }
} 
